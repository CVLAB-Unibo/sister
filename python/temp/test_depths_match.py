...
for f in frames_paths:
    ...
    output_path = [x for x in glob.glob(os.path.join(f, 'output', '*.png')) if
                   args.tag == os.path.splitext(os.path.basename(x))[0]]
    ...
    if args.filter_bounds:
        ...
        depth = np.clip(depth, 0, float(baseline) * camera.getFx())  # max_gt*10)
